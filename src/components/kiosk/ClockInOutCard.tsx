  ...
  return (
    <Card className="w-full max-w-lg mx-auto shadow-2xl">
      <CardHeader className="text-center">
        <Clock className="mx-auto h-16 w-16 text-primary mb-2" />
        <CardTitle className="text-5xl font-mono font-bold tracking-wider">{currentTime || "Loading..."}</CardTitle>
        {branch ? (
          <p className="text-muted-foreground text-lg">Branch: {branch}</p>
        ) : (
      ...
      <CardContent className="space-y-6 p-6">
        <div className="space-y-2">
          <Label htmlFor="employeeCodeKiosk" className="text-lg font-semibold">Employee Code</Label>
          <Input
            ...
            className="text-center text-2xl h-16 rounded-lg"
            aria-label="Employee Code"
          />
        </div>
        
        <div className="grid grid-cols-2 gap-4">
          <Button onClick={() => handleClockAction('in')} className="text-xl py-8 rounded-lg bg-green-600 hover:bg-green-700 text-white">
            <LogIn className="mr-2 h-6 w-6" /> Clock In
          </Button>
          <Button onClick={() => handleClockAction('out')} className="text-xl py-8 rounded-lg bg-red-600 hover:bg-red-700 text-white">
            <LogOut className="mr-2 h-6 w-6" /> Clock Out
          </Button>
        </div>
      </CardContent>
      <CardFooter className="flex flex-col items-center justify-center p-6 bg-muted/50 rounded-b-lg">
        <div className="flex items-center text-muted-foreground mb-2">
          <Camera className="h-6 w-6 mr-2" />
          <p className="text-sm">Photo will be captured upon clocking in/out.</p>
        </div>
        <div data-ai-hint="photo capture area" className="w-full h-24 bg-gray-200 border-2 border-dashed border-gray-400 rounded-md flex items-center justify-center text-gray-500">
          Photo Capture Area
        </div>
      </CardFooter>
    </Card>
